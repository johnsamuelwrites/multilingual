...
class KeywordRegistryTestSuite(unittest.TestCase):
    ...
    def test_get_keyword_portuguese(self):
        ...
        self.assertEqual(self.registry.get_keyword("PRINT", "pt"), "imprima")



    def test_portuguese_diacritic_and_legacy_aliases(self):

        """Portuguese should support diacritic-first canonicals plus legacy aliases."""

        # Canonical forms

        self.assertEqual(

            self.registry.get_keyword("COND_ELSE", "pt"),
        ...
        self.assertEqual(
            ...
            self.registry.get_keyword("DEFAULT", "pt"),

            "padr\u00e3o",

        )

        self.assertEqual(self.registry.get_keyword("NOT", "pt"), "n\u00e3o")

        self.assertEqual(

            self.registry.get_keyword("TYPE_DICT", "pt"),

            "dicion\u00e1rio",

        )

        self.assertEqual(self.registry.get_keyword("TYPE_STR", "pt"), "texto")

        self.assertEqual(self.registry.get_keyword("TYPE_FLOAT", "pt"), "real")



        # Legacy/ascii aliases still resolve

        self.assertEqual(self.registry.get_concept("senao", "pt"), "COND_ELSE")

        self.assertEqual(self.registry.get_concept("padrao", "pt"), "DEFAULT")

        self.assertEqual(self.registry.get_concept("nao", "pt"), "NOT")

        self.assertEqual(self.registry.get_concept("dicionario", "pt"), "TYPE_DICT")

        self.assertEqual(self.registry.get_concept("imprimir", "pt"), "PRINT")
