...
دالة معاملات_متعددة(base, extra=1, *args, **kwargs):
    إرجاع base + extra + sum(args)
ليكن نتيجة_متعددة = معاملات_متعددة(10, 2, 3, 4, key=5)

# Lambda
ليكن مربع = لامدا x: x * x

# List/dict comprehensions و generator expression
ليكن قائمة_ج = [x * 2 لكل x في range(4)]
ليكن قاموس_ج = {str(k): k * k لكل k في range(3)}
ليكن مولد_ج = list(x + 1 لكل x في range(3))
ليكن متداخل_ج = [i + j لكل i في range(2) لكل j في range(2)]
ليكن مرشح_ج = [x لكل x في range(6) إذا x % 2 == 0]
...
# Decorator
دالة مضاعف(func):
    دالة غلاف(*args, **kwargs):
        إرجاع func(*args, **kwargs) * 2
    إرجاع غلاف
...
print(رقم_سداسي, رقم_ثماني, رقم_ثنائي, رقم_علمي)
print(موسع, بت_و, بت_او, بت_اكس, بت_يسار, بت_يمين)
print(سلسلة_ا, سلسلة_ب, سلسلة_ج)
print(منوع, مشروح(3, 1.5), ثلاثي)
print(نتيجة_متعددة, مربع(5))
print(قائمة_ج, قاموس_ج, مولد_ج)
print(متداخل_ج, مرشح_ج)
print(حاول_والا, متسلسل, استثناءات_متعددة)
print(نتيجة_مطابقة, نتيجة_مزخرف, خاصية)
print(مع_توثيق())
